CREATE TABLE [content].[gatherableTypes] (
    [typeId]                     TINYINT         NOT NULL,
    [type]                       VARCHAR (255)   NOT NULL,
    [typeName]                   VARCHAR (255)   NOT NULL,
    [description]                VARCHAR (1000)  NOT NULL,
    [parentEnum]                 VARCHAR (50)    NOT NULL,
    [parentTypeId]               TINYINT         NOT NULL,
    [childEnum]                  VARCHAR (50)    NOT NULL,
    [globalObjectNamingType]     SMALLINT        NOT NULL,
    [experienceMultiplierPlayer] DECIMAL (5, 2)  NULL,
    [experienceMultiplierSkill]  DECIMAL (18, 2) NOT NULL,
    CONSTRAINT [gatherableTypes_pk] PRIMARY KEY CLUSTERED ([typeId] ASC)
);


GO

CREATE UNIQUE NONCLUSTERED INDEX [gatherableTypes_gatherableTypeId_uindex]
    ON [content].[gatherableTypes]([typeId] ASC);


GO


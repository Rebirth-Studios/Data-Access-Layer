CREATE FUNCTION [dbo].[getMaxRarityName](@globalObject VARCHAR(255))
RETURNS VARCHAR(255)
AS
BEGIN
    DECLARE @maxRarityName VARCHAR(255)
	DECLARE @maxRarityId TINYINT
	DECLARE @itemTypeName VARCHAR(255)
	DECLARE @isImbued BIT

	Set @maxRarityName = 'None';
	...
	SELECT @maxRarityName = typeName
	FROM [content].[scriptableRarities]
	WHERE typeId = @maxRarityId


    RETURN @maxRarityName
END

...
IF Not EXISTS(SELECT 1 FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_NAME = 'Internal_FK_Definition_Storage')
  ...
  CREATE TABLE [ops].[Internal_FK_Definition_Storage] 
  (
...
ELSE
   BEGIN
    ...
    TRUNCATE TABLE [ops].[Internal_FK_Definition_Storage]    
      END
      ELSE
...
IF @Recycle = 0
  ...
  WHILE (@i <= (SELECT MAX(ID) FROM #FKs))
   BEGIN
    ...
    INSERT INTO [ops].[Internal_FK_Definition_Storage]
    ...
    -- DROP FOREING KEYS
    SET @i = 1
    WHILE (@i <= (SELECT MAX(ID) FROM [ops].[Internal_FK_Definition_Storage]))
          BEGIN
             SET @ConstraintName = (SELECT FK_Name FROM [ops].[Internal_FK_Definition_Storage] WHERE ID = @i)
    SET @Statement = (SELECT FK_DestructionStatement FROM [ops].[Internal_FK_Definition_Storage] WITH (NOLOCK) WHERE ID = @i)
...
-- to resolve this the stored procedure should be called recursively, but I dont have the time to do it...          
 /*
    ...
    SET @Statement = (SELECT Table_TruncationStatement FROM [ops].[Internal_FK_Definition_Storage] WHERE ID = @i)
...
*/          
	...
	from sys.tables
	WHERE name = @TableToTruncate

	PRINT '4.1 memory_optimized'
	IF @memory_optimized = 0
		EXEC('TRUNCATE TABLE [' + @TableToTruncate + ']')
	ELSE
		EXEC('DELETE FROM [' + @TableToTruncate + ']')
    ...
    WHILE (@i <= (SELECT MAX(ID) FROM [ops].[Internal_FK_Definition_Storage]))
          BEGIN
             SET @ConstraintName = (SELECT FK_Name FROM [ops].[Internal_FK_Definition_Storage] WHERE ID = @i)
    SET @Statement = (SELECT FK_CreationStatement FROM [ops].[Internal_FK_Definition_Storage] WHERE ID = @i)

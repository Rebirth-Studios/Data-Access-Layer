...
CREATE VIEW [dbo].[containerTypesView] AS 
	...
	SELECT 
		...
		--JOIN [content].[containerTypes] gt ON it.typeId = gt.parentTypeId
		--LEFT JOIN [content].[containerClassificationTypes] gct ON gt.typeId = gct.parentTypeId
		--LEFT JOIN [content].[containerSubTypes] gst ON gct.typeId = gst.parentTypeId
	WHERE it.type = 'Container'
